...
def auto_approval(request: RequestInfo) -> str:
    """
    Return a message
    Args:
        request (RequestInfo): Request Info input

    Returns:
        str: A message based on request
    """
    ft = "FULL-TIME" if request.is_fulltime is True else "INTERN"
    return f"Employee {request.first_name} {request.last_name}({ft}) with a request of tuition {request.tuition_amount} and over all grade {request.overall_grade} is automatically APPROVED."
