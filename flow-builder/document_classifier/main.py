...
from examples.flow_builder.document_classifier.tools.document_classifier_flow import build_docclassifier_flow
...
async def main():
    ...
    my_flow_definition = await build_docclassifier_flow().compile_deploy()
    generated_folder = f"{Path(__file__).resolve().parent}/generated"
    my_flow_definition.dump_spec(f"{generated_folder}/docclassifier_flow.json")
    global flow_run
